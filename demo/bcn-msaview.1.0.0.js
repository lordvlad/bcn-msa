...
var request$5281 = require('./request'), MSA$5282 = require('./MSASource'), extend$5283 = require('extend'), colorSchemeSelector$5284 = require('biojs-util-colorschemes').selector;
/**
 * Helpers
 */
var floor$5286 = Math.floor.bind(Math);
var ceil$5288 = Math.ceil.bind(Math);
function minmaxval$5290(min$5293, max$5294, val$5295) {
    if (typeof min$5293 !== 'undefined' && val$5295 < min$5293)
        return min$5293;
    if (typeof max$5294 !== 'undefined' && val$5295 > max$5294)
        return max$5294;
    return val$5295;
}
/**
 ...
 */
function MSAView$5291(root$5296, options$5297) {
    window.a = this;
    var canvas$5299;
    // include default options and options from DOM dataset
    options$5297 = extend$5283({}, MSAView$5291.defaultOptions, root$5296.dataset, options$5297);
    // create canvas if not present
    if (!(canvas$5299 = root$5296.querySelector('canvas')))
        root$5296.appendChild(canvas$5299 = document.createElement('canvas'));
    if ('bcnMsaFullscreen' in options$5297)
        root$5296.style.width = root$5296.style.height = '100%';
    // set canvas proportions and hide cursor
    canvas$5299.width = root$5296.offsetWidth;
    canvas$5299.height = root$5296.offsetHeight;
    // canvas.style.cursor = 'none';
    // convenience method
    canvas$5299.on = function (__fa_args$5302, event$5303, callback$5304) {
        return canvas$5299.addEventListener(event$5303, callback$5304.bind(this));
    }.bind(this, typeof arguments !== 'undefined' ? arguments : undefined);
    // attach event handlers
    canvas$5299.on('mousewheel', this.onScroll);
    canvas$5299.on('wheel', this.onScroll);
    canvas$5299.on('mousemove', this.onPointerMove);
    canvas$5299.on('mouseout', this.onPointerOut);
    canvas$5299.on('mousedown', this.onPointerDown);
    canvas$5299.on('mouseup', this.onPointerUp);
    canvas$5299.on('contextmenu', function (__fa_args$5306, e$5307) {
        return e$5307.preventDefault();
    }.bind(this, typeof arguments !== 'undefined' ? arguments : undefined));
    this.alignment = new MSA$5282(root$5296.dataset.alignment);
    this.scrollPos = {
        x: 0,
        y: 0,
        maxX: 0,
    ...
    this.ctx = canvas$5299.getContext('2d');
    this.ctx.font = options$5297.font;
    this.draw = this.draw.bind(this);
    this.render = this.draw.bind(this);
    this.colorScheme = colorSchemeSelector$5284.getColor(options$5297.colorScheme);
    this.options = options$5297;
    this.canvas = canvas$5299;
    this.mousePos = null;
    this.LOCK = false;
    this.view = null;
    this.updateView();
...
MSAView$5291.prototype.updateView = function updateView() {
    if (this.LOCK)
        return this.LOCK;
    if (!this.view)
        return Promise.resolve();
    var cvs$5310 = this.canvas, aln$5311 = this.alignment, opt$5312 = this.options, view$5313 = this.view, scroll$5314 = this.scrollPos, em$5315 = view$5313.charWidth + opt$5312.letterSpacing, H$5316 = cvs$5310.clientHeight, W$5317 = cvs$5310.clientWidth;
    return this.LOCK = Promise.all([
        aln$5311.getLines(view$5313.offsetY, view$5313.offsetY + view$5313.height),
        aln$5311.getSize()
    ]).then(function (res$5318) {
        view$5313.tracks = res$5318[0];
        view$5313.alignment = res$5318[1].alignment;
        view$5313.count = res$5318[1].sequenceCount;
        view$5313.sequenceWidth = res$5318[1].sequenceWidth;
        scroll$5314.maxX = floor$5286(view$5313.sequenceWidth * em$5315) - W$5317 + opt$5312.labelWidth + opt$5312.leftMargin;
        scroll$5314.maxY = (res$5318[1].sequenceCount - view$5313.height) * opt$5312.lineHeight - H$5316;
        view$5313.lastOffsetY = view$5313.offsetY;
        this.LOCK = false;
    }.bind(this));
};
MSAView$5291.prototype.onScroll = function onScroll(e$5319) {
    if (this.LOCK)
        return;
    var s$5322 = this.scrollPos, dx$5323 = e$5319.deltaX || -e$5319.wheelDeltaX, dy$5324 = e$5319.deltaY || -e$5319.wheelDeltaY;
    s$5322.x = floor$5286(minmaxval$5290(0, s$5322.maxX, s$5322.x + dx$5323));
    s$5322.y = floor$5286(minmaxval$5290(0, s$5322.maxY, s$5322.y + dy$5324));
};
MSAView$5291.prototype.onPointerOut = function onPointerOut() {
    this.mousePos = null;
};
MSAView$5291.prototype.onPointerMove = function onPointerMove(e$5325) {
    var r$5327 = this.canvas.getBoundingClientRect(), m$5328 = this.mousePos || {};
    this.mousePos = {
        left: m$5328.left,
        right: m$5328.right,
        middle: m$5328.middle,
        y: e$5325.clientY - r$5327.top,
        x: e$5325.clientX - r$5327.left
    };
};
MSAView$5291.prototype.onPointerDown = function onPointerDown(e$5329) {
    var m$5331 = this.mousePos;
    if (!m$5331)
        return;
    switch (e$5329.which) {
    case 1:
...
MSAView$5291.prototype.onPointerUp = function onPointerUp(e$5332) {
    var m$5334 = this.mousePos;
    if (m$5334.middle)
...
MSAView$5291.prototype.draw = function draw(t$5335) {
    var cvs$5337 = this.canvas, ctx$5338 = this.ctx, opt$5339 = this.options, view$5340 = this.view, lock$5341 = this.LOCK, mouse$5342 = this.mousePos, scroll$5343 = this.scrollPos, col$5344 = this.colorScheme, em$5345 = view$5340 && view$5340.charWidth + opt$5339.letterSpacing, charWidth$5346 = view$5340 && view$5340.charWidth, H$5347 = cvs$5337.clientHeight, W$5348 = cvs$5337.clientWidth, lineHeight$5349 = opt$5339.lineHeight, letterSpacing$5350 = opt$5339.letterSpacing, labelWidth$5351 = opt$5339.labelWidth, leftMargin$5352 = opt$5339.leftMargin;
    if (!view$5340) {
        view$5340 = this.view = { tracks: [] };
        charWidth$5346 = view$5340.charWidth = ceil$5288(ctx$5338.measureText('x').width);
        em$5345 = view$5340.charWidth + letterSpacing$5350;
        view$5340.height = floor$5286(H$5347 / lineHeight$5349) - 4;
        view$5340.seqOffset = labelWidth$5351 + letterSpacing$5350 + leftMargin$5352;
        view$5340.labelTruncate = floor$5286((labelWidth$5351 - leftMargin$5352) / charWidth$5346);
    }
    // check mouse middle button scroll
    if (mouse$5342 && mouse$5342.middle) {
        var dx$5355 = (mouse$5342.x - scroll$5343.pan.x) / 10, dy$5356 = (mouse$5342.y - scroll$5343.pan.y) / 10;
        scroll$5343.x = floor$5286(minmaxval$5290(0, scroll$5343.maxX, scroll$5343.x + dx$5355));
        scroll$5343.y = floor$5286(minmaxval$5290(0, scroll$5343.maxY, scroll$5343.y + dy$5356));
    }
    view$5340.offsetX = floor$5286(scroll$5343.x / em$5345);
    view$5340.offsetY = floor$5286(scroll$5343.y / lineHeight$5349);
    // get mouse coordinates relative to sequence and
    // aminoacid position
    if (mouse$5342) {
        mouse$5342.sx = floor$5286((mouse$5342.x - view$5340.seqOffset) / em$5345) + view$5340.offsetX;
        mouse$5342.sy = floor$5286(mouse$5342.y / lineHeight$5349) - 2 + view$5340.offsetY;
    }
    // maybe we need to fetch new lines from the underlying source
    if (!lock$5341 && view$5340.offsetY !== view$5340.lastOffsetY)
        this.updateView();
    // clear canvas
    ctx$5338.clearRect(0, 0, W$5348, H$5347);
    // loading text
    if (lock$5341)
        ctx$5338.fillText(opt$5339.loadingText, leftMargin$5352, lineHeight$5349);
    // ruler
    {
        var y$5359 = lineHeight$5349 * 2, x$5360 = view$5340.seqOffset, i$5361 = view$5340.offsetX + 1;
        while (x$5360 < W$5348) {
            if (i$5361 % 10 === 0 || i$5361 === 1)
                ctx$5338.fillText(i$5361 + '', x$5360, y$5359);
            else if (i$5361 % 5 === 0)
                ctx$5338.fillText('.', x$5360, y$5359);
            i$5361 += 1;
            x$5360 += em$5345;
        }
    }
    // tracks
    if (view$5340 && view$5340.tracks) {
        var bgW$5364 = em$5345 + 2, bgH$5365 = lineHeight$5349 + 2, bgX$5366 = -em$5345 / 4, bgY$5367 = -lineHeight$5349 * 4 / 5, m$5368 = mouse$5342;
        view$5340.tracks.forEach(function (t$5369, i$5370) {
            var y$5373 = (i$5370 + 3) * lineHeight$5349, x$5374 = view$5340.seqOffset, j$5375 = view$5340.offsetX, s$5376 = t$5369.sequence, l$5377 = t$5369.label;
            // sequence
            while (x$5374 < W$5348) {
                var a$5380 = s$5376[j$5375];
                if ((aa = s$5376[j$5375]) !== '-' && col$5344[aa]) {
                    ctx$5338.beginPath();
                    ctx$5338.fillStyle = col$5344[aa];
                    ctx$5338.strokeStyle = col$5344[aa];
                    ctx$5338.rect(x$5374 + bgX$5366, y$5373 + bgY$5367, em$5345, lineHeight$5349);
                    ctx$5338.fill();
                    ctx$5338.stroke();
                    ctx$5338.closePath();
                    ctx$5338.fillStyle = 'black';
                }
                ctx$5338.fillText(aa, x$5374, y$5373);
                j$5375 += 1;
                x$5374 += em$5345;
            }
            // label
            if (m$5368 && mouse$5342.sx < view$5340.offsetX && mouse$5342.sy === view$5340.offsetY + i$5370) {
                ctx$5338.save();
                ctx$5338.fillStyle = 'rgb(255, 255, 255)';
                ctx$5338.fillRect(leftMargin$5352, y$5373 - lineHeight$5349 + 2, W$5348, lineHeight$5349 + 4);
                ctx$5338.restore();
            } else {
                l$5377 = l$5377.substr(0, view$5340.labelTruncate) + '...';
            }
            ctx$5338.fillText(l$5377, leftMargin$5352, y$5373);
        });
    }
    if (mouse$5342) {
        var r$5383 = mouse$5342.left || mouse$5342.right || mouse$5342.middle ? 6 : 4;
        // info line
        if (view$5340.tracks && view$5340.tracks[mouse$5342.sy]) {
            var seq$5386 = view$5340.tracks[mouse$5342.sy], pos$5387 = seq$5386.sequence[mouse$5342.sx];
            if (pos$5387 && pos$5387 !== '-') {
                pos$5387 += seq$5386.sequence.substr(0, mouse$5342.sx).replace(/-/g, '').length + 1;
                ctx$5338.fillText(pos$5387, view$5340.seqOffset, lineHeight$5349);
            }
            ctx$5338.fillText(seq$5386.label, view$5340.seqOffset + 6 * charWidth$5346, lineHeight$5349);
        }
    }    // ctx.save();
...
function fillCircle$5292(x$5388, y$5389, r$5390, ctx$5391) {
    ctx$5391.beginPath();
    ctx$5391.arc(x$5388, y$5389, r$5390, 0, Math.PI * 2);
    ctx$5391.fill();
    ctx$5391.closePath();
}
MSAView$5291.create = function (options$5392, domNode$5393) {
    return new MSAView$5291(domNode$5393, options$5392);
};
MSAView$5291.defaultOptions = {
    font: '12px monospace',
    lineHeight: 14,
    labelWidth: 100,
    leftMargin: 20,
