...
var request$1318 = require('./request'), MSA$1319 = require('./MSASource'), extend$1320 = require('extend'), colorSchemeSelector$1321 = require('biojs-util-colorschemes').selector;
/**
 * Helpers
 */
var floor$1323 = Math.floor.bind(Math);
var ceil$1325 = Math.ceil.bind(Math);
function minmaxval$1326(mi$1333, ma$1334, val$1335) {
    if (val$1335 < mi$1333)
        return mi$1333;
    if (val$1335 > ma$1334)
        return ma$1334;
...
function slice$1327(arr$1336, a$1337, b$1338) {
    return Array.prototype.slice.call(arr$1336, a$1337, b$1338);
}
function $$1328(q$1339, r$1340) {
    return (r$1340 || document).querySelector(q$1339);
}
function $$$1330(q$1341, r$1342) {
    if (q$1341.constructor.name !== 'NodeList')
        q$1341 = (r$1342 || document).querySelectorAll(q$1341);
    return slice$1327(q$1341);
}
/**
 * MultiSequenceAlignment Viewer class
 */
function MSAView$1331(root$1343, options$1344) {
    window.a = this;
    var canvas$1346;
    // include default options and options from DOM dataset
    options$1344 = extend$1320({}, MSAView$1331.defaultOptions, root$1343.dataset, options$1344);
    // create canvas if not present
    if (!(canvas$1346 = $$1328('canvas', root$1343)))
        root$1343.appendChild(canvas$1346 = document.createElement('canvas'));
    if ('bcnMsaFullscreen' in options$1344)
        root$1343.style.width = root$1343.style.height = '100%';
    // set canvas proportions and hide cursor
    canvas$1346.width = root$1343.offsetWidth;
    canvas$1346.height = root$1343.offsetHeight;
    // canvas.style.cursor = 'none';
    // convenience method
    canvas$1346.on = function (__fa_args$1349, event$1350, callback$1351) {
        return canvas$1346.addEventListener(event$1350, callback$1351.bind(this));
    }.bind(this, typeof arguments !== 'undefined' ? arguments : undefined);
    // attach event handlers
    canvas$1346.on('mousewheel', this.onScroll);
    canvas$1346.on('wheel', this.onScroll);
    canvas$1346.on('mousemove', this.onPointerMove);
    canvas$1346.on('mouseout', this.onPointerOut);
    canvas$1346.on('mousedown', this.onPointerDown);
    canvas$1346.on('mouseup', this.onPointerUp);
    canvas$1346.on('contextmenu', function (__fa_args$1353, e$1354) {
        return e$1354.preventDefault();
    }.bind(this, typeof arguments !== 'undefined' ? arguments : undefined));
    this.alignment = new MSA$1319(root$1343.dataset.alignment);
    this.scrollPos = {
        x: 0,
        y: 0,
        maxX: 0,
    ...
    this.ctx = canvas$1346.getContext('2d');
    this.ctx.font = options$1344.font;
    this.draw = this.draw.bind(this);
    this.render = this.draw.bind(this);
    this.colorScheme = colorSchemeSelector$1321.getColor(options$1344.colorScheme);
    this.options = options$1344;
    this.canvas = canvas$1346;
    this.mousePos = null;
    this.LOCK = false;
    this.view = null;
    this.updateView();
...
MSAView$1331.prototype.updateView = function updateView() {
    if (this.LOCK)
        return this.LOCK;
    if (!this.view)
        return Promise.resolve();
    var cvs$1357 = this.canvas, aln$1358 = this.alignment, opt$1359 = this.options, view$1360 = this.view, scroll$1361 = this.scrollPos, em$1362 = view$1360.charWidth + opt$1359.letterSpacing, H$1363 = cvs$1357.clientHeight, W$1364 = cvs$1357.clientWidth;
    return this.LOCK = Promise.all([
        aln$1358.getLines(view$1360.offsetY, view$1360.offsetY + view$1360.height),
        aln$1358.getSize()
    ]).then(function (res$1365) {
        view$1360.tracks = res$1365[0];
        view$1360.alignment = res$1365[1].alignment;
        view$1360.count = res$1365[1].sequenceCount;
        view$1360.sequenceWidth = res$1365[1].sequenceWidth;
        scroll$1361.maxX = floor$1323(view$1360.sequenceWidth * em$1362) - W$1364 + opt$1359.labelWidth + opt$1359.leftMargin;
        scroll$1361.maxY = (res$1365[1].sequenceCount - view$1360.height) * opt$1359.lineHeight - H$1363;
        view$1360.lastOffsetY = view$1360.offsetY;
        this.LOCK = false;
    }.bind(this));
};
MSAView$1331.prototype.onScroll = function onScroll(e$1366) {
    if (this.LOCK)
        return;
    var s$1369 = this.scrollPos, dx$1370 = e$1366.deltaX || -e$1366.wheelDeltaX, dy$1371 = e$1366.deltaY || -e$1366.wheelDeltaY;
    s$1369.x = floor$1323(minmaxval$1326(0, s$1369.maxX, s$1369.x + dx$1370));
    s$1369.y = floor$1323(minmaxval$1326(0, s$1369.maxY, s$1369.y + dy$1371));
};
MSAView$1331.prototype.onPointerOut = function onPointerOut() {
    this.mousePos = null;
};
MSAView$1331.prototype.onPointerMove = function onPointerMove(e$1372) {
    var r$1374 = this.canvas.getBoundingClientRect(), m$1375 = this.mousePos || {};
    this.mousePos = {
        left: m$1375.left,
        right: m$1375.right,
        middle: m$1375.middle,
        y: e$1372.clientY - r$1374.top,
        x: e$1372.clientX - r$1374.left
    };
};
MSAView$1331.prototype.onPointerDown = function onPointerDown(e$1376) {
    var m$1378 = this.mousePos;
    if (!m$1378)
        return;
    switch (e$1376.which) {
    case 1:
...
MSAView$1331.prototype.onPointerUp = function onPointerUp(e$1379) {
    var m$1381 = this.mousePos;
    if (m$1381.middle)
...
MSAView$1331.prototype.draw = function draw(t$1382) {
    var cvs$1384 = this.canvas, ctx$1385 = this.ctx, opt$1386 = this.options, view$1387 = this.view, lock$1388 = this.LOCK, mouse$1389 = this.mousePos, scroll$1390 = this.scrollPos, col$1391 = this.colorScheme, em$1392 = view$1387 && view$1387.charWidth + opt$1386.letterSpacing, charWidth$1393 = view$1387 && view$1387.charWidth, H$1394 = cvs$1384.clientHeight, W$1395 = cvs$1384.clientWidth, lineHeight$1396 = opt$1386.lineHeight, letterSpacing$1397 = opt$1386.letterSpacing, labelWidth$1398 = opt$1386.labelWidth, leftMargin$1399 = opt$1386.leftMargin;
    if (!view$1387) {
        view$1387 = this.view = { tracks: [] };
        charWidth$1393 = view$1387.charWidth = ceil$1325(ctx$1385.measureText('x').width);
        em$1392 = view$1387.charWidth + letterSpacing$1397;
        view$1387.height = floor$1323(H$1394 / lineHeight$1396) - 4;
        view$1387.seqOffset = labelWidth$1398 + letterSpacing$1397 + leftMargin$1399;
        view$1387.labelTruncate = floor$1323((labelWidth$1398 - leftMargin$1399) / charWidth$1393);
    }
    // check mouse middle button scroll
    if (mouse$1389 && mouse$1389.middle) {
        var dx$1402 = (mouse$1389.x - scroll$1390.pan.x) / 10, dy$1403 = (mouse$1389.y - scroll$1390.pan.y) / 10;
        scroll$1390.x = floor$1323(minmaxval$1326(0, scroll$1390.maxX, scroll$1390.x + dx$1402));
        scroll$1390.y = floor$1323(minmaxval$1326(0, scroll$1390.maxY, scroll$1390.y + dy$1403));
    }
    view$1387.offsetX = floor$1323(scroll$1390.x / em$1392);
    view$1387.offsetY = floor$1323(scroll$1390.y / lineHeight$1396);
    // get mouse coordinates relative to sequence and
    // aminoacid position
    if (mouse$1389) {
        mouse$1389.sx = floor$1323((mouse$1389.x - view$1387.seqOffset) / em$1392) + view$1387.offsetX;
        mouse$1389.sy = floor$1323(mouse$1389.y / lineHeight$1396) - 2 + view$1387.offsetY;
    }
    // maybe we need to fetch new lines from the underlying source
    if (!lock$1388 && view$1387.offsetY !== view$1387.lastOffsetY)
        this.updateView();
    // clear canvas
    ctx$1385.clearRect(0, 0, W$1395, H$1394);
    // loading text
    if (lock$1388)
        ctx$1385.fillText(opt$1386.loadingText, leftMargin$1399, lineHeight$1396);
    // ruler
    {
        var y$1406 = lineHeight$1396 * 2, x$1407 = view$1387.seqOffset, i$1408 = view$1387.offsetX + 1;
        while (x$1407 < W$1395) {
            if (i$1408 % 10 === 0 || i$1408 === 1)
                ctx$1385.fillText(i$1408 + '', x$1407, y$1406);
            else if (i$1408 % 5 === 0)
                ctx$1385.fillText('.', x$1407, y$1406);
            i$1408 += 1;
            x$1407 += em$1392;
        }
    }
    // tracks
    if (view$1387 && view$1387.tracks) {
        var bgW$1411 = em$1392 + 2, bgH$1412 = lineHeight$1396 + 2, bgX$1413 = -em$1392 / 4, bgY$1414 = -lineHeight$1396 * 4 / 5, m$1415 = mouse$1389;
        view$1387.tracks.forEach(function (t$1416, i$1417) {
            var y$1420 = (i$1417 + 3) * lineHeight$1396, x$1421 = view$1387.seqOffset, j$1422 = view$1387.offsetX, s$1423 = t$1416.sequence, l$1424 = t$1416.label;
            // sequence
            while (x$1421 < W$1395) {
                var a$1427 = s$1423[j$1422];
                if ((aa = s$1423[j$1422]) !== '-' && col$1391[aa]) {
                    ctx$1385.beginPath();
                    ctx$1385.fillStyle = col$1391[aa];
                    ctx$1385.strokeStyle = col$1391[aa];
                    ctx$1385.rect(x$1421 + bgX$1413, y$1420 + bgY$1414, em$1392, lineHeight$1396);
                    ctx$1385.fill();
                    ctx$1385.stroke();
                    ctx$1385.closePath();
                    ctx$1385.fillStyle = 'black';
                }
                ctx$1385.fillText(aa, x$1421, y$1420);
                j$1422 += 1;
                x$1421 += em$1392;
            }
            // label
            if (m$1415 && mouse$1389.sx < view$1387.offsetX && mouse$1389.sy === view$1387.offsetY + i$1417) {
                ctx$1385.save();
                ctx$1385.fillStyle = 'rgb(255, 255, 255)';
                ctx$1385.fillRect(leftMargin$1399, y$1420 - lineHeight$1396 + 2, W$1395, lineHeight$1396 + 4);
                ctx$1385.restore();
            } else {
                l$1424 = l$1424.substr(0, view$1387.labelTruncate) + '...';
            }
            ctx$1385.fillText(l$1424, leftMargin$1399, y$1420);
        });
    }
    if (mouse$1389) {
        var r$1430 = mouse$1389.left || mouse$1389.right || mouse$1389.middle ? 6 : 4;
        // info line
        if (view$1387.tracks && view$1387.tracks[mouse$1389.sy]) {
            var seq$1433 = view$1387.tracks[mouse$1389.sy], pos$1434 = seq$1433.sequence[mouse$1389.sx];
            if (pos$1434 && pos$1434 !== '-') {
                pos$1434 += seq$1433.sequence.substr(0, mouse$1389.sx).replace(/-/g, '').length + 1;
                ctx$1385.fillText(pos$1434, view$1387.seqOffset, lineHeight$1396);
            }
            ctx$1385.fillText(seq$1433.label, view$1387.seqOffset + 6 * charWidth$1393, lineHeight$1396);
        }
    }    // ctx.save();
...
function fillCircle$1332(x$1435, y$1436, r$1437, ctx$1438) {
    ctx$1438.beginPath();
    ctx$1438.arc(x$1435, y$1436, r$1437, 0, Math.PI * 2);
    ctx$1438.fill();
    ctx$1438.closePath();
}
MSAView$1331.create = function (options$1439, domNode$1440) {
    return new MSAView$1331(domNode$1440, options$1439);
};
MSAView$1331.defaultOptions = {
    font: '12px monospace',
    lineHeight: 14,
    labelWidth: 100,
    leftMargin: 20,
...
$$$1330('[data-bcn="msa"]').map(MSAView$1331.create.bind(null, null));
